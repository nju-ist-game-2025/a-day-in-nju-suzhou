    ...
    if (!entry.isCharacter) {
        ...
        if (!entry.phaseImages.isEmpty()) {
            ...
            for (const PhaseImage &phase: entry.phaseImages) {
                ...
                phaseImageLabel->setStyleSheet(
                        "background-color: rgba(255, 255, 255, 200); border: 1px solid rgba(100, 149, 237, 150); border-radius: 8px;");
...
void Codex::loadBossData() {
    ...
    nightmare.weakness = QString("炸弹闹钟可对其造成%1点伤害").arg(
            config.getEnemyInt("clock_boom", "explosion_damage_nightmare", 50));
    ...
    teacher.skills = QString(
            "【正态分布弹幕】发射%1发弹幕，角度服从N(μ,%2°)\n【随机点名】在玩家位置生成延时伤害红圈\n【极大似然估计】预判玩家移动方向放置陷阱\n【喜忧参半】发射在2/3距离处分裂成%3发的大型弹幕")
...
void Codex::loadEnemyData() {
    ...
    clockNormal.attackMethod = QString("近战接触攻击（%1点伤害）").arg(
            config.getEnemyInt("clock_normal", "contact_damage", 2));
    ...
    sockNormal.attackMethod = QString("近战接触攻击（%1点伤害）").arg(
            config.getEnemyInt("sock_normal", "contact_damage", 1));
    ...
    sockAngrily.attackMethod = QString("冲刺攻击（%1点伤害）").arg(
            config.getEnemyInt("sock_angrily", "contact_damage", 2));
    ...
    pants.attackMethod = QString("近战接触攻击（%1点伤害）+ 旋转技能").arg(
            config.getEnemyInt("pants", "contact_damage", 2));
    ...
    digitalSystem.attackMethod = QString("近战接触攻击（%1点伤害）").arg(
            config.getEnemyInt("digital_system", "contact_damage", 3));
    ...
    optimization.attackMethod = QString("近战接触攻击（%1点伤害）").arg(
            config.getEnemyInt("optimization", "contact_damage", 3));
    ...
    invigilator.skills = QString(
            "【巡逻】围绕奶牛张Boss环形巡逻（半径%1）\n【警觉】发现玩家后切换为愤怒状态（检测范围%2）\n【冲刺】愤怒状态下快速冲向玩家（速度%3）")
